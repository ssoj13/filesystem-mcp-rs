...
Use `locate_search` for repeated searches by file name or path. It accepts `path` or `paths`. Searching never scans: a path nobody indexed returns no matches with `indexState: "unindexed"` and a hint, so index it first with `locate_refresh` (or `bgnd_scan_ctl start`) and poll `locate_status`. `waitMs` only waits for a scan that is already running. `bgnd_scan_ctl` stops a scan (its rows are kept, a later start resumes it), pauses it in place, or lists every root and its progress; it works across MCP processes. All roots must be allowed by the server. `grep_files` searches file contents instead.

Use `locate_sql` for what a name search cannot say: the largest files under a folder, size per extension, folders that hold two given entries (Unity projects, Rust projects with a `target`), `node_modules` not nested in another. It takes one `SELECT` over two views, `fs_entries(path, name, dir, ext, kind, size, modified, files, dirs, depth, under)` and `fs_roots(path, state, last_verified, last_error)`, limited to the directories the server allows. A directory's `size` is the total beneath it. The `under` parameter narrows `fs_entries` to one folder and everything beneath it (through the primary key, so there is no path arithmetic to get wrong), and `dir = under` lists that folder's children. Without it a subtree of `P` is `path >= 'P\' AND path < 'P]'`; filters on `dir`, `ext` and `depth` scan the subtree. `help: true` returns the columns, the functions (`fs_ext`, `REGEXP`) and ten ready recipes (largest files, what takes the space in a folder, size per extension, old big files, duplicates by name, Unity projects and their `Library`, Cargo `target` folders, non-nested `node_modules`); `saveTo` writes every row to a file as JSON lines; `timeoutMs` (default 120000) stops a runaway query. A question over millions of rows takes seconds to tens of seconds; a range on `path` keeps it small. Nothing else is reachable: writes, pragmas, `ATTACH`, other tables and functions that allocate without bound are refused, and the connection is `query_only`. The answers are as fresh as `roots[].lastVerified`.
...
- **`"bash"` on Windows is git-bash**, resolved from the installed git (`git --exec-path`, then the usual install locations) instead of taking whatever `PATH` offers first. `C:\Windows\System32\bash.exe` is the WSL launcher and normally precedes git-bash on `PATH`; running a command through it crosses the Win32→WSL boundary, so variables and loops expand on the Linux side and come back empty, the `env` map never arrives and Windows paths mean nothing. If the only bash found is that launcher the call **fails with an error naming it** rather than silently using it — use `shell: "pwsh"` or install git-bash.
