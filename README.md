...
- Indexed names: `locate_search` (several roots, exact/prefix/contains/glob/regex, file/directory kind and include/exclude fragments), `locate_refresh` (queue a scan), `locate_status` (asynchronous progress), `bgnd_scan_ctl` (status / start / stop / pause / resume the scans)
