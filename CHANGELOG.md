...
### Locate: upgrading, refresh cost and how the changes were checked

- **Additive schema.** v10 adds one table (`dir_stats`) and no column. An older binary ignores it. Nothing heavy runs at startup: the migration is a `CREATE TABLE`, and the totals are built by the worker, in bounded commits, never in `init_schema` (a migration that held the writer lock for minutes would make every other process fail to open the index).
- **Mixed versions.** Only the process that owns the scan lock runs the backfill. While an older process owns it, a root published by that process has no totals for its new attempt; `locate_search` then reports directory size 0 with `null` counts, and `locate_sql` shows the directory's own (zero) size. They appear when a process of this version takes the lock.
- **What a scan costs.** The first scan of a volume was measured at about 2 h 35 min for 11.9 M entries (1.6 M directories); it builds two trigram full-text indexes as it goes and paces its own writes. A refresh walks the volume again, stages every row in a table without full-text indexes and applies only the difference; it has not been timed. Totals add a pass over the visited entries in memory (one map of directories). `FS_MCP_LOCATE_EXCLUDE` shortens both.
- **How it was checked.** Unit tests cover the roll-up (including `a-b` sorting between `a` and `a\b`), refresh, partial refresh, backfill, recovery and every refused statement of `locate_sql`. Two ignored tests run against a copy of a real index and are kept for the next change: `real_index_totals_match_the_entries_they_summarise` (`LOCATE_DB_COPY=<dir>`), `real_index_sql_answers_typical_questions_in_reasonable_time` (also `LOCATE_SQL_FILE=<queries>` for your own questions and `LOCATE_SQL_PLAN=1|only` for query plans), run with `--release -- --ignored --nocapture`.

### `locate_sql`

- **Read-only SQL over the index**, for the questions a name search cannot answer: largest files under a folder, size per extension, folders holding two given entries, `node_modules` not nested in another. One `SELECT` over two temporary views, `fs_entries` (path, name, dir, ext, kind, size, modified, files, dirs, depth; a directory's `size` is its subtree total) and `fs_roots`. The views hide generations and covered roots and are already limited to the directories the server allows.
- **`under` and recipes.** The one thing a caller got wrong was the path arithmetic for a subtree (`path >= 'P\' AND path < 'P]'`). The `under` parameter does it: it narrows `fs_entries` to one folder and what lies beneath it, intersected with the directories the server allows, through the same primary-key range; the `under` column holds the folder so `dir = under` lists its children. `help: true` now returns ten recipes written for `under`; a test runs every recipe in the guide against a real index, so the guide cannot drift from the views. The guide moved to `sql_guide.txt`.
- **Fenced in**: the connection is `query_only`; an authorizer refuses everything except a `SELECT` over the two views (writes, pragmas, `ATTACH`, every other table, `zeroblob`/`randomblob`/`load_extension`); a progress handler stops a query at `timeoutMs` (default 120 s) and says so; one statement only. The `regexp` operator and `fs_ext()` are registered on the connection. `saveTo` writes every row as JSON lines (rows are streamed, never held), `help: true` returns the guide and example queries.
- **Measured on a real index (11.9 M entries, cold to warm cache):** anything that seeks by name or path answers at once (Unity projects by two sibling names 0.0 s, non-nested `node_modules` by size 0.1 s, `target` folders by size 0.0 s); a sweep of a subtree takes seconds (largest files under a 2 M-entry folder 7.6 s, children of a folder by size 5.8 s), and a whole-volume aggregate over 10 M files takes about 30 s (`size per extension`). The first version joined `roots` in the view and SQLite then read the generation as a range and swept every index instead of seeking (20 to 92 s for those questions); each root's generation is now written into its own `UNION ALL` arm as a constant. The connection also maps the file and keeps a 256 MB cache. A covering index over `(kind, path, name, size, modified)` would make the sweeps sequential, at the price of a multi-minute index build under the writer lock; it is not done.
- **A structured query language was considered and dropped.** A JSON expression grammar does not fit the tool surface budget (`tool_surface_guard`: 600 characters of description, 2000 with the schema), and SQL is the notation the caller already knows: the whole tool costs one short description and five small parameters. `rusqlite` gains the `functions` and `hooks` features (no new crates).
...
- **An interrupted first scan is resumed, not discarded.** Until now, when the process that owned the scan died (a client closed, a crash) or the server shut down, recovery deleted every row the scan had written and the next attempt started from zero. On a whole-drive scan that lost millions of rows several times. A scan of a never-published root now keeps its rows under `roots.resume_generation` (schema v8) and the next attempt continues that generation with an upsert that rewrites only rows whose metadata changed; shutdown ends an attempt as `interrupted`, without the failure backoff. Recovery still drops generations nothing can resume or serve. Refreshes are unchanged: their staging is cheap to redo. Known limit: a file deleted between the interruption and the resume stays in the published index until the next reconciliation.
- **Less disk churn while scanning.** `synchronous=NORMAL` (WAL), a larger checkpoint interval with a WAL size limit, 2000-row scan commits, and a pause after each commit proportional to how long it took (10 ms to 2 s) instead of a fixed 20 ms. One measured whole-drive scan wrote 385 GB to disk to build an 11 GB index.
- **Scan progress is logged.** Scan start (resumed or not), progress every 60 s, completion, interruption and failure now go through `tracing` into the per-process log.
- **`bgnd_scan_ctl`** (`status | start | stop | pause | resume`, optional `path`; schema v9, `roots.control`). The request is written to the index, so it reaches a scan running in another MCP process: `stop` ends the running scan within a fraction of a second and keeps its rows for a later `start`; `pause` holds it in place, including during the NTFS tree read; a stopped or paused root is not claimed by the worker.
