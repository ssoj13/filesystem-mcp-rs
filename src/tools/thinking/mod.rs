...
impl ThinkingState {
    ...
    pub fn process(&self, input: ThoughtInput) -> ThoughtOutput {
        ...
        // Store in history (recover from poisoned mutex if needed)
        match self.thought_history.lock() {
            Ok(mut history) => history.push(data.clone()),
            Err(poisoned) => poisoned.into_inner().push(data.clone()),
        }

        // Store in branch if applicable
        if let (Some(_branch_from), Some(branch_id)) = (data.branch_from_thought, &data.branch_id) {
            match self.branches.lock() {
                Ok(mut branches) => {
                    branches
        ...
        // Build output (recover from poisoned mutex if needed)
        let history_len = match self.thought_history.lock() {
            Ok(h) => h.len(),
            Err(poisoned) => poisoned.into_inner().len(),
        };
        let branch_keys: Vec<String> = match self.branches.lock() {
            Ok(b) => b.keys().cloned().collect(),
    ...
    fn log_thought(&self, data: &ThoughtData) {
        let (prefix, context) = if data.is_revision == Some(true) {
            (
                "REVISION",
                format!(" (revising thought {})", data.revises_thought.unwrap_or(0)),
            )
