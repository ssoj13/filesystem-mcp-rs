...
impl WatchEvent {
    pub fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            _ => bail!(
                "Unknown event type '{}'. Supported: modify, create, delete",
                s
            ),
        }
    }
...
pub async fn tail_file(path: &Path, params: TailParams) -> Result<TailResult> {
    ...
    if let Some(bytes) = params.bytes {
        ...
        // Seeking `bytes` from the end can land mid-codepoint; drop the leading
        // UTF-8 continuation bytes so the window starts on a char boundary and
        // the whole tail is not mis-decoded as some legacy encoding.
        let skip = buf.iter().take_while(|&&b| (b & 0xC0) == 0x80).count();
        let content = decode_bytes(&buf[skip..]);
...
async fn tail_follow(
    ...
    // Shutdown signal: dropping `stop_tx` unblocks the blocking watcher thread
    // immediately. spawn_blocking work cannot be cancelled by JoinHandle::abort,
    // so a parked thread would otherwise linger for the full timeout after we
    // return, holding a blocking-pool slot.
    ...
    let watcher_handle = tokio::task::spawn_blocking(move || {
        ...
        // Hold the watcher alive until shutdown is signalled or the safety
        // timeout elapses.
        let _ = stop_rx.recv_timeout(Duration::from_millis(timeout_ms + 1000));
        Ok::<_, anyhow::Error>(())
    });

    // Wait for file change or timeout
    let wait_result = timeout(Duration::from_millis(timeout_ms), rx.recv()).await;

    // Signal the watcher thread to exit, then let it finish promptly.
    drop(stop_tx);
...
pub async fn watch_file(
    ...
    let watcher_handle = tokio::task::spawn_blocking(move || {
        ...
        // We watch the PARENT directory (to catch create/delete), so sibling
        // files in that directory also produce events. Filter to the target by
        // file name so an unrelated file changing does not report a false hit.
        let is_dir = path_buf.is_dir();
        let target_name = path_buf.file_name().map(|n| n.to_os_string());
        let mut watcher = RecommendedWatcher::new(
            move |res: Result<Event, notify::Error>| {
                if let Ok(event) = res {
                    let concerns_target = is_dir
                        || event
                            .paths
                            .iter()
                    ...
                    let matched = match event.kind {
                        EventKind::Modify(_) => {
                            events.is_empty() || events.contains(&WatchEvent::Modify)
                        }
                        EventKind::Create(_) => {
                            events.is_empty() || events.contains(&WatchEvent::Create)
                        }
                        EventKind::Remove(_) => {
                            events.is_empty() || events.contains(&WatchEvent::Delete)
                        }
                        _ => false,
                    };
        ...
        // Hold the watcher alive until shutdown is signalled (see tail_follow)
...
mod tests {
    ...
    async fn test_watch_file_modify() {
        ...
        let result = watch_file(&path, 2000, &[WatchEvent::Modify])
            .await
            .unwrap();
    ...
    async fn test_watch_file_timeout() {
        ...
        // Wait a bit for filesystem to settle (macOS FSEvents can fire on subscribe)
        sleep(Duration::from_millis(200)).await;

        let result = watch_file(&path, 300, &[WatchEvent::Modify]).await.unwrap();

        assert!(!result.changed);
        assert!(result.timed_out);
    }

    // BH-19: watching a file must ignore changes to a SIBLING file in the same
    // directory. We watch the parent dir to catch create/delete, so sibling
    // events arrive and must be filtered out by path.
    #[tokio::test]
    async fn test_watch_file_ignores_sibling() {
        let dir = tempdir().unwrap();
        let target = dir.path().join("target.txt");
        let sibling = dir.path().join("sibling.txt");
        fs::write(&target, "x").await.unwrap();
        sleep(Duration::from_millis(200)).await;
        ...
        tokio::spawn(async move {
            for i in 0..5 {
                ...
                let _ = fs::write(&sib, format!("change {i}")).await;
            }
        });

        let result = watch_file(&target, 600, &[WatchEvent::Modify])
            .await
            .unwrap();
        assert!(
            !result.changed,
            "a sibling file changing must not trigger the target watch"
        );
        assert!(result.timed_out);
    }
    ...
    async fn test_watch_event_from_str() {
        assert!(matches!(
            WatchEvent::from_str("modify").unwrap(),
            WatchEvent::Modify
        ));
        assert!(matches!(
            WatchEvent::from_str("CREATE").unwrap(),
            WatchEvent::Create
        ));
        assert!(matches!(
            WatchEvent::from_str("delete").unwrap(),
            WatchEvent::Delete
        ));
