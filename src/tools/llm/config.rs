...
impl Config {
    ...
    pub fn apply_env_overrides(&mut self) {
        ...
        if self.openai_api_key.trim().is_empty()
            && let Ok(value) = std::env::var("OPENAI_API_KEY")
            && !value.trim().is_empty()
        {
            self.openai_api_key = value;
        }
    ...
    pub fn effective_api_key_for(&self, provider: &str) -> Option<String> {
        if provider.eq_ignore_ascii_case(&self.provider)
            && let Some(key) = &self.provider_api_key
            && !key.trim().is_empty()
        {
    ...
    pub fn effective_endpoint_for(&self, provider: &str) -> String {
        if provider.eq_ignore_ascii_case(&self.provider)
            && let Some(endpoint) = &self.provider_endpoint
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.effective_endpoint_for(&provider).trim().is_empty() {
            bail!("LLM_MCP_PROVIDER_ENDPOINT is required for non-gemini providers");
        }
...
pub fn config_path() -> Result<PathBuf> {
    let exe = std::env::current_exe().context("Failed to locate binary path")?;
    let dir = exe.parent().context("Failed to resolve binary directory")?;
    let stem = exe
        .file_stem()
        .and_then(|s| s.to_str())
        .context("Failed to determine binary name")?;
    Ok(dir.join(format!("{}.toml", stem)))
}

// The `Parsed` variant carries the full TOML config + key set (~400 bytes);
// the other variants are small. Boxing would force a heap alloc for the
// common path — not worth it for an internal enum that lives briefly.
...
fn read_config_file(path: &Path) -> Result<ConfigReadOutcome> {
    if !path.exists() {
...
fn write_config_file(path: &Path, config: &Config, keys: Option<&HashSet<String>>) -> Result<()> {
    let include_all = keys.is_none();
    let should_include = |key: &str| include_all || keys.is_some_and(|set| set.contains(key));
    let mut table = toml::map::Map::new();

    if should_include("LLM_MCP_PROVIDER") {
        table.insert(
            "LLM_MCP_PROVIDER".to_string(),
            toml::Value::String(config.provider.clone()),
        );
    }
    ...
    if should_include("LLM_MCP_PROVIDER_ENDPOINT")
        && let Some(endpoint) = &config.provider_endpoint
    {
        table.insert(
            "LLM_MCP_PROVIDER_ENDPOINT".to_string(),
            toml::Value::String(endpoint.clone()),
        );
    }
    if should_include("LLM_MCP_PROVIDER_API_KEY")
        && let Some(key) = &config.provider_api_key
    {
        table.insert(
            "LLM_MCP_PROVIDER_API_KEY".to_string(),
    ...
    if should_include("LLM_MCP_GEMINI_API_KEY") {
        table.insert(
            "LLM_MCP_GEMINI_API_KEY".to_string(),
            toml::Value::String(config.gemini_api_key.clone()),
        );
    }
    if should_include("LLM_MCP_CEREBRAS_API_KEY") {
        table.insert(
            "LLM_MCP_CEREBRAS_API_KEY".to_string(),
            toml::Value::String(config.cerebras_api_key.clone()),
        );
    }
    if should_include("LLM_MCP_OPENAI_API_KEY") {
        table.insert(
            "LLM_MCP_OPENAI_API_KEY".to_string(),
            toml::Value::String(config.openai_api_key.clone()),
        );
    }
    if should_include("LLM_MCP_BIG_MODEL") {
        table.insert(
            "LLM_MCP_BIG_MODEL".to_string(),
            toml::Value::String(config.big_model.clone()),
        );
    }
    if should_include("LLM_MCP_SMALL_MODEL") {
        table.insert(
            "LLM_MCP_SMALL_MODEL".to_string(),
            toml::Value::String(config.small_model.clone()),
        );
    }
    ...
    let output =
        toml::to_string_pretty(&toml::Value::Table(table)).context("Failed to serialize config")?;
    fs::write(path, output).context("Failed to write config")?;
    Ok(())
}
