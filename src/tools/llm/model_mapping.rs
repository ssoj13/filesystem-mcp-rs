...
impl ModelManager {
    pub fn new(config: &Config) -> Self {
        ...
        let mut gemini_models = base_models
            .iter()
            .map(|m| m.to_string())
            .collect::<HashSet<_>>();
...
mod tests {
    ...
    fn maps_known_aliases_to_size_models() {
        ...
        let (mapped, ok) = manager.validate_and_map_model(
            "claude-3-5-sonnet-20241022",
            "gemini",
            ModelMappingMode::Auto,
        );
        ...
        let (mapped, ok) =
            manager.validate_and_map_model("gpt-4o", "gemini", ModelMappingMode::Auto);
