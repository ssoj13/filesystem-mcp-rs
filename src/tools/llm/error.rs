pub fn classify_provider_error(error_msg: &str) -> String {
    ...
    if error_lower.contains("error parsing chunk")
        && error_lower.contains("expecting property name")
    {
        return "Streaming parsing error (malformed JSON chunk). Retry or disable streaming."
            .to_string();
    }
    ...
    if error_lower.contains("parsing")
    ...
    if error_lower.contains("safety")
        || (error_lower.contains("content") && error_lower.contains("filter"))
    {
        return "ContentBlock filtered by provider safety systems. Modify the request.".to_string();
    }

    if error_lower.contains("token")
        && (error_lower.contains("limit") || error_lower.contains("exceed"))
    {
