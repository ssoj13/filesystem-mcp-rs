...
pub async fn read_pdf(path: &Path, pages: Option<&str>, max_chars: usize) -> Result<PdfReadResult> {
    let path_buf = path.to_path_buf();
    let pages_owned = pages.map(|s| s.to_string());

    // pdf-extract is sync, run in blocking task
    tokio::task::spawn_blocking(move || read_pdf_sync(&path_buf, pages_owned.as_deref(), max_chars))
        .await
...
fn read_pdf_sync(
    ...
    // Select the requested pages (if any), then apply the character budget.
    let (mut result_text, pages_extracted) = match pages_str {
        Some(range) => {
            let selected = parse_page_range(range, total_pages)?;
            let picked = select_page_text(&text, &selected, total_pages)?;
            (picked, selected)
        }
        None => (text, (1..=total_pages).collect::<Vec<_>>()),
    };
...
mod tests {
    ...
    // BH-22: page selection must return ONLY the requested pages, not all text.
    #[test]
    ...
    fn test_select_page_text_errors_without_separators() {
        // total_pages estimated (>1) but no form feeds -> cannot isolate pages.
