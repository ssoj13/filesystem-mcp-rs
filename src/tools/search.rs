...
/// Build exclude set from patterns (re-exported for compatibility)
...
mod tests {
    ...
    async fn test_search_files_only() {
        ...
        let results = search_files_extended(&params, &allowed, false)
            .await
            .unwrap();
    ...
    fn test_file_type_filter_from_str() {
        ...
        assert_eq!(
            FileTypeFilter::from_str("directory"),
            Some(FileTypeFilter::Dir)
        );
        assert_eq!(
            FileTypeFilter::from_str("symlink"),
            Some(FileTypeFilter::Symlink)
        );
