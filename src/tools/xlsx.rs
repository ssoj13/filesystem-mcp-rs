...
pub fn xlsx_info(path: &Path) -> Result<Value> {
...
/// List sheet names
pub fn xlsx_sheets(path: &Path) -> Result<Vec<String>> {
    let workbook = open_workbook_auto(path).context("Failed to open workbook")?;
...
mod tests {
    ...
    fn create_test_xlsx() -> NamedTempFile {
        ...
        sheet.write_number(2, 1, 3.5).unwrap();
    ...
    fn test_xlsx_info() {
        ...
        assert!(!json["sheets"].as_array().unwrap().is_empty());
    }
    ...
    fn test_xlsx_read_with_headers() {
        ...
        assert!(
            headers
                .iter()
                .any(|h| h.as_str().unwrap().contains("Описание"))
        );
    }
