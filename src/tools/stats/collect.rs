...
//! the process. Nothing here touches a database, a clock or the filesystem; [`super::write`] is
//! the only thing that ever reads the map, once, on the way out.
//!
//! Four rules decide the shape, each of them paid for:
//!
//! 1. **Every tool the router serves has a row from the moment the process starts**, at zero.
//!    That is what lets the file answer "which tools does nobody use": a map that grew a row on
//!    first use could only ever list the tools that *were* used, and the interesting half of the
//!    answer - the silent tail - would be indistinguishable from a tool that had been removed.
//! 2. **The key is the tool name and nothing else.** There is no time dimension: one file per
//!    run is the resolution, and slicing a run more finely than that is what the flush this
//!    design replaced existed to do.
//! 3. **The tool name is interned against the router's own list** and anything else becomes
...
//! 4. **[`Collector::record_interned`] cannot fail**: no `Result`, no I/O, and no allocation at
//!    all for a tool the router serves, because rule 1 already put its row there. It takes a
//!    mutex, adds integers, releases.
//!
...
impl Collector {
    /// Intern `names` - the router's tool list - and give every one of them a row of zeros.
    ///
    /// Seeded rather than grown on demand, which is rule 1 and the whole reason the unused tail
    /// is reportable at all. It also means [`Collector::record_interned`] never allocates for a
    /// tool the router serves: the entry it merges into is already there.
    pub fn new<I, S>(names: I) -> Self
    where
        I: IntoIterator<Item = S>,
        S: AsRef<str>,
    {
        let known: HashSet<Arc<str>> = names.into_iter().map(|n| Arc::from(n.as_ref())).collect();
        // `__unknown` is deliberately NOT seeded: a row of zeros for it would appear in every
        // file ever written and mean nothing, whereas its presence says a client called a name
        // this build does not serve.
        let rows: Table = known
            .iter()
            .map(|name| (Arc::clone(name), Counts::default()))
            .collect();
        Self {
            known,
    ...
    /// A copy of the table, or `None` if the counter lock is held right now.
    ///
    /// Never blocks, and that is the point. The only reader is [`super::write`], and on the panic
    /// path it runs on the thread that panicked - possibly the very thread holding this lock. A
    /// blocking read there would hang the process while it was trying to report a crash, so the
    /// reader is given the means to say "I could not read it" instead. Losing one file is a far
    /// better outcome than a deadlock mid-crash.
    ///
    /// A poisoned lock still yields its data: the critical section is integer arithmetic on a map
    /// entry, so a panic elsewhere while it happened to be held leaves the counters consistent.
    pub fn try_snapshot(&self) -> Option<Table> {
        match self.rows.try_lock() {
            Ok(rows) => Some(rows.clone()),
            Err(std::sync::TryLockError::Poisoned(rows)) => Some(rows.into_inner().clone()),
            Err(std::sync::TryLockError::WouldBlock) => None,
        }
    }

    /// Hold the counter lock, so a test can exercise the path where the table cannot be read.
    #[cfg(test)]
    pub fn hold_for_test(&self) -> std::sync::MutexGuard<'_, Table> {
        self.lock()
    }
...
mod tests {
    ...
    /// A collector that serves two tools, which is all most of these tests need.
    fn collector() -> Collector {
        Collector::new(["read_text_file", "grep_files"])
    }

    /// The table, which every test but one may simply read: nothing else holds the lock.
    fn table(c: &Collector) -> Table {
        c.try_snapshot().expect("the counter lock is free")
    }
    ...
    fn two_calls_to_one_tool_merge_into_one_row() {
        ...
        let table = table(&c);
        assert_eq!(table.len(), 2, "both tools have a row from the start");
        assert_eq!(
    ...
    fn unknown_names_collapse_into_one_key() {
        ...
        assert_eq!(
            table.len(),
            3,
            "the two seeded tools, plus one key shared by all three typos"
        );
        assert_eq!(
    ...
    /// a reader that emptied the table would make the file it wrote a lie.
    #[test]
    fn a_snapshot_leaves_the_table_intact() {
        let c = collector();
        record(&c, "read_text_file", Outcome::Ok, 5, 2);

        assert_eq!(only_row(&table(&c), "read_text_file").ns_total, 5);
        assert_eq!(
            only_row(&table(&c), "read_text_file").ns_total,
            5,
    ...
    fn an_unknown_shape_is_stored_as_deferred_but_counted_apart() {
        ...
        assert_eq!(
            only_row(&table(&c), "grep_files").deferred,
            2,
    ...
    fn every_tool_the_router_serves_interns_to_itself() {
        ...
        assert_eq!(
            table(&c).len(),
            names.len(),
            "every tool the router serves must have a row before any call arrives"
        );
        for name in &names {
            assert_eq!(
                &*c.intern(name),
    ...
    /// While the lock is held the table cannot be read, and the reader is told so rather than
    /// blocking - which on the panic path would hang the process mid-crash.
    #[test]
    fn a_held_lock_yields_nothing_rather_than_blocking() {
        let c = collector();
        let held = c.hold_for_test();
        assert!(c.try_snapshot().is_none(), "the lock is held");
        drop(held);
        assert!(c.try_snapshot().is_some(), "and free again once released");
    }
