...
impl HashAlgorithm {
    ...
    pub fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            _ => bail!(
                "Unknown algorithm '{}'. Supported: md5, sha1, sha256, sha512, xxh64, murmur3, spooky",
                s
            ),
        }
    }
...
pub async fn hash_files_multiple(paths: &[&Path], algorithm: HashAlgorithm) -> MultiHashResult {
...
mod tests {
    ...
    async fn test_hash_file_range_offset() {
        ...
        // Hash "world" (offset 6)
        let result = hash_file_range(&path, HashAlgorithm::Sha256, Some(6), None)
            .await
            .unwrap();
    ...
    async fn test_hash_file_range_offset_and_length() {
        ...
        let result = hash_file_range(&path, HashAlgorithm::Sha256, Some(2), Some(3))
            .await
            .unwrap();
    ...
    async fn test_hash_file_range_offset_beyond_file() {
        ...
        let result = hash_file_range(&path, HashAlgorithm::Sha256, Some(100), None)
            .await
            .unwrap();
    ...
    async fn test_hash_file_range_length_exceeds_file() {
        ...
        let result = hash_file_range(&path, HashAlgorithm::Sha256, Some(2), Some(100))
            .await
            .unwrap();
    ...
    async fn test_hash_files_multiple_identical() {
        ...
        let result =
            hash_files_multiple(&[path1.as_path(), path2.as_path()], HashAlgorithm::Sha256).await;
    ...
    async fn test_algorithm_from_str() {
        ...
        assert!(matches!(
            HashAlgorithm::from_str("xxhash64").unwrap(),
            HashAlgorithm::Xxh64
        ));
        assert!(matches!(
            HashAlgorithm::from_str("murmur3").unwrap(),
            HashAlgorithm::Murmur3
        ));
        assert!(matches!(
            HashAlgorithm::from_str("murmur").unwrap(),
            HashAlgorithm::Murmur3
        ));
        assert!(matches!(
            HashAlgorithm::from_str("spooky").unwrap(),
            HashAlgorithm::Spooky
        ));
        assert!(matches!(
            HashAlgorithm::from_str("spookyhash").unwrap(),
            HashAlgorithm::Spooky
        ));
    ...
    async fn test_hash_file_murmur3_with_offset() {
        ...
        let result = hash_file_range(&path, HashAlgorithm::Murmur3, Some(6), None)
            .await
            .unwrap();
