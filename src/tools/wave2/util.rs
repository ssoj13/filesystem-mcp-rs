...
pub fn clipboard_read() -> Result<String> {
    bail!("Clipboard support requires screenshot-tools feature")
}
...
#[cfg(not(feature = "screenshot-tools"))]
pub fn clipboard_write(_text: &str) -> Result<()> {
    bail!("Clipboard support requires screenshot-tools feature")
}
...
pub fn which(command: &str) -> Result<Value> {
    ...
    #[cfg(target_os = "windows")]
    let pathext =
        env::var("PATHEXT").unwrap_or_else(|_| ".COM;.EXE;.BAT;.CMD;.VBS;.JS;.WS;.MSC".to_string());
    #[cfg(target_os = "windows")]
    ...
    if abs_path.is_absolute()
        && abs_path.is_file()
        && !found.contains(&abs_path.to_string_lossy().to_string())
    {
