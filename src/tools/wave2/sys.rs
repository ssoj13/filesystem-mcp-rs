...
pub fn disk_usage(path: Option<&Path>) -> Result<Value> {
    ...
    if let Some(target_path) = path {
        ...
        bail!("No disk found for path: {}", target_path.display());
    }
...
pub fn sys_info() -> Value {
    ...
    json!({
        ...
        "memory": {
            ...
            "total_human": format_size(total_mem),
            "used_human": format_size(used_mem)
        },
        "swap": {
            "total_bytes": total_swap,
            "used_bytes": used_swap,
            "total_human": format_size(total_swap),
            "used_human": format_size(used_swap)
        },
        "load_average": {
            "one": load_avg.one,
            "five": load_avg.five,
