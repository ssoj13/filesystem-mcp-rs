...
use crate::tools::binary::from_base64;
...
pub async fn list_buckets(client: &Client) -> Result<Vec<S3BucketInfo>> {
    let resp = client
        ...
        .send()
        .await
        .context("S3 list_buckets failed")?;
...
pub async fn get_object(client: &Client, params: S3GetParams) -> Result<S3GetResult> {
    ...
    if let Some(path) = &params.output_path {
        // Never publish a truncated object as a downloaded file: the result
        // would look like success while the file on disk is incomplete (BH-17).
        if truncated {
            bail!(
                "S3 object {}/{} exceeded maxBytes ({}); refusing to write a truncated file. \
                 Raise maxBytes or omit it to fetch the whole object.",
                params.bucket,
                params.key,
                params.max_bytes.unwrap_or(0)
            );
        }
        fs::write(path, &body)
            .await
...
fn decode_body(body: &Option<String>, body_base64: bool) -> Result<Option<Vec<u8>>> {
    ...
    if body_base64 {
        let bytes = from_base64(body).context("Invalid base64 body")?;
        Ok(Some(bytes))
    } else {
...
pub async fn presign(client: &Client, params: S3PresignParams) -> Result<String> {
    let expires =
        PresigningConfig::expires_in(std::time::Duration::from_secs(params.expires_in_seconds))
