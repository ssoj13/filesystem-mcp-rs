...
use crate::core::glob::build_glob_set;
use crate::tools::hash::{HashAlgorithm, hash_file_range};
...
pub async fn compare_directories(
    ...
    let ignore = build_glob_set(&params.ignore_patterns)?;

    // Collect files from both directories. Enumeration errors are threaded into
    // `errors` so a tree that could not be fully read never reports identical.
    let mut errors = Vec::new();
    let files1 = collect_files(path1, path1, params.recursive, &ignore, &mut errors).await?;
    let files2 = collect_files(path2, path2, params.recursive, &ignore, &mut errors).await?;
    ...
    for rel_path in common {
        ...
        if params.compare_content {
            match (
                hash_file(&info1.full_path).await,
                hash_file(&info2.full_path).await,
            ) {
                (Ok(h1), Ok(h2)) => {
                    if h1 != h2 {
                        different.push(DirDiffFile {
...
async fn collect_files(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let meta = match entry.metadata().await {
            Ok(m) => m,
            Err(e) => {
                // Surface the failure rather than silently dropping the entry:
                // a skipped file would otherwise be absent from both sides and
                // wrongly counted as "identical".
                errors.push(format!("{}: {}", rel_path, e));
                continue;
            }
        };
        ...
        if meta.is_file() {
            ...
            let sub = Box::pin(collect_files(root, &path, recursive, ignore, errors)).await?;
            result.extend(sub);
        }
    }
...
mod tests {
    ...
    async fn test_compare_identical_files() {
        ...
        let result = compare_files(&path1, &path2, CompareParams::default())
            .await
            .unwrap();
    ...
    async fn test_compare_dirs_identical() {
        ...
        let result = compare_directories(&dir1, &dir2, DirCompareParams::default())
            .await
            .unwrap();
    ...
    async fn test_compare_dirs_different_size() {
        ...
        fs::write(dir2.join("file.txt"), b"much longer")
            .await
            .unwrap();
    ...
    async fn test_compare_dirs_ignore_pattern() {
        ...
        fs::write(dir1.join("ignore.log"), b"different1")
            .await
            .unwrap();
        fs::write(dir2.join("file.txt"), b"same").await.unwrap();
        fs::write(dir2.join("ignore.log"), b"different2")
            .await
            .unwrap();
