...
impl ArchiveFormat {
    ...
    pub fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            _ => bail!(
                "Unknown archive format '{}'. Supported: zip, tar, tar.gz",
                s
            ),
        }
    }
...
fn extract_zip(
    ...
    let mut archive = zip::ZipArchive::new(file).with_context(|| "Invalid ZIP archive")?;
    ...
    for i in 0..archive.len() {
        ...
        if let Some(filter) = files_filter
            && !filter.iter().any(|f| name.starts_with(f) || name == *f)
        {
            continue;
        }
...
pub async fn create_archive(
    ...
    let destination = destination.to_path_buf();
    let format = format
        .or_else(|| ArchiveFormat::from_path(&destination))
        .unwrap_or(ArchiveFormat::Zip);

    tokio::task::spawn_blocking(move || create_archive_sync(&paths, &destination, format)).await?
}
...
mod tests {
    ...
    fn test_format_from_str() {
        assert!(matches!(
            ArchiveFormat::from_str("zip").unwrap(),
            ArchiveFormat::Zip
        ));
        assert!(matches!(
            ArchiveFormat::from_str("TAR").unwrap(),
            ArchiveFormat::Tar
        ));
        assert!(matches!(
    ...
    async fn test_create_and_extract_zip() {
        ...
        let result = extract_archive(&archive_path, &extract_dir, None, None)
            .await
            .unwrap();
    ...
    async fn test_create_and_extract_tar_gz() {
        ...
        let result = create_archive(&[src_file], &archive_path, Some(ArchiveFormat::TarGz))
            .await
            .unwrap();
    ...
    async fn test_extract_with_filter() {
        ...
        let archive_path = dir.path().join("test.zip");
        create_archive(&[src_dir], &archive_path, Some(ArchiveFormat::Zip))
            .await
            .unwrap();
