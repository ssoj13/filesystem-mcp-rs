...
pub async fn read_json(path: &Path, query: Option<&str>, pretty: bool) -> Result<JsonReadResult> {
    // Read file content
...
fn apply_query(json: &Value, query: &str) -> Result<(Value, bool)> {
    ...
    for part in parts {
        ...
        if let Some(idx_start) = part.find('[') {
            // Validate bracket syntax: need at least "[N]"
            if !part.ends_with(']') || part.len() < idx_start + 3 {
                return Ok((Value::Null, false));
            }
...
fn extract_error_context(content: &str, line: usize, column: usize) -> Option<String> {
    ...
    {
        context.push(format!("{:4} | {}", i + 1, line));
    }
...
mod tests {
    ...
    async fn test_read_valid_json_object() {
        ...
        fs::write(&path, r#"{"name": "test", "value": 42}"#)
            .await
            .unwrap();
    ...
    async fn test_read_invalid_json() {
        ...
        fs::write(&path, r#"{"name": "test", invalid}"#)
            .await
            .unwrap();
    ...
    async fn test_query_dot_notation() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("test.json");
        fs::write(&path, r#"{"user": {"name": "Alice", "age": 30}}"#)
            .await
            .unwrap();
    ...
    async fn test_query_jsonpath() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("test.json");
        fs::write(&path, r#"{"users": [{"name": "Alice"}, {"name": "Bob"}]}"#)
            .await
            .unwrap();

        let result = read_json(&path, Some("$.users[*].name"), true)
            .await
            .unwrap();
