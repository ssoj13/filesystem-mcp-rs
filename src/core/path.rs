            ...
            if !is_within_allowed(&real, &allowed_snapshot) {
                if allow_symlink_escape {
                    ...
                    if meta.file_type().is_symlink() {
                        // Canonicalize the parent dir (not following the symlink itself)
                        // to handle cases like /tmp -> /private/tmp on macOS
                        if let Some(parent) = normalized.parent() {
                            ...
                            if is_within_allowed(&canonical_link, &allowed_snapshot) {
        ...
        Err(err) => {
            ...
            if err.kind() == std::io::ErrorKind::NotFound {
                ...
                let parent = existing_parent.ok_or_else(|| {
                    anyhow!("Parent directory does not exist: {}", normalized.display())
                })?;
                let parent_real = fs::canonicalize(&parent).await.with_context(|| {
