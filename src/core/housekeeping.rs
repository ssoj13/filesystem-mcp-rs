...
//!
//! One mechanism serves `<state>/tmp` now and the statistics database in a later wave: two
//! separate retention implementations would drift apart and each would need its own concurrency
//! story. A second caller needs nothing from this module but a different `kind` string for the
//! lease and a different directory for [`sweep_dir`], both of which are `pub(crate)`.
//!
//! **Log files are deliberately not swept.** Every run writes one file under `<state>/logs` and
//! nothing here ever deletes it; the operator clears that directory by hand.
//!
...
mod tests {
    ...
    /// The marker is named after the kind, so a second kind of housekeeping is not silenced for
    /// an hour by the scratch sweep that ran a moment earlier.
    #[test]
    fn leases_are_independent_per_kind() {
        ...
        assert_eq!(
            lease_due(root, "other", HOUR, now),
            Lease::Due,
            "another kind is a separate lease"
        );
    }
