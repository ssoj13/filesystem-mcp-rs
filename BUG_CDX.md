...
Verification after restart (2026-09-25): the installed binary matched the release build. The previous broad-root work was persisted in the old index database; after stopping all server instances and resetting only `everything.db` plus its WAL/SHM files, `locate_status` reported `C:\`, `D:\`, and `D:\projects` as unindexed with no queued work. `locate_refresh({path:"D:\\projects",waitMs:0})` queued only `\\?\D:\Projects`; a concurrent `locate_search` and idempotent refresh returned immediately without a lock error. The scan completed in about 92 seconds with 316,743 entries and 33,016 directories. `locate_search` for exact `Cargo.toml` returned matches; `C:\` and `D:\` stayed unindexed. The new database was 246,865,920 bytes and had no WAL/SHM at the final check.
